<div id="x-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal">
    <div x-data="_livewireModal()" x-on:open-x-modal.window="onOpen($event)" x-on:modal-ready.window="ready = true"
        x-init="boot()" class="modal-dialog" :class="[size ? `modal-${size}` : '']" role="document">
...
<script>
    window._livewiremodal = window._livewiremodal || {
        theme: "{{ config('livewiremodal.theme') ?? 'bs5' }}"
    }
</script>
