function _livewireModal() {
    return {
      ...
      boot() {
        ...
        if (_livewiremodal.theme === "bs5") {
          document
            .getElementById("x-modal")
            .addEventListener("hidden.bs.modal", () => closeModal());
        } else if (_livewiremodal.theme === "bs4") {
          $("#x-modal").on("hidden.bs.modal", () => closeModal());
        }
      },
      onOpen(event) {
        this.heading = event.detail.title;
        this.modal = event.detail.modal;
        this.size = Object.prototype.hasOwnProperty.call(event.detail, "size")
          ? event.detail.size
          : null;
        this.ready = false;
  ...
  function _openModal(title, modal, params = [], size = null) {
    window.dispatchEvent(
      new CustomEvent("open-x-modal", {
        detail: { title: title, modal: modal, size: size, args: params },
      })
    );
  }
